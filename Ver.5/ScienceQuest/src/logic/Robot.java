...
public class Robot {
	...
	public Robot(){
		...
		//Initially, the character is standing still with its head turned right
		currentFrame=idle_R;
		
		//Initialise the bounding box of the character, this will be very important
		//when managing collisions between objects
		boundingBox=new Rectangle(ROBOT_START_X+DISPLACEMENT,currentY,ROBOT_WIDTH,ROBOT_HEIGHT);
	}
	...
	public void checkBlockCollisions(){
		...
		//if the character is jumping, their head must not touch a block;
		...
		if((boundingBox.getMaxX()/Tile.TILE_SIZE)<World.COLS){
			if(World.tiledMap[currentRow][currentCol] instanceof Collectible){
				if(World.tiledMap[currentRow][currentCol].getBoundingBox().intersects(boundingBox)){
					if(World.tiledMap[currentRow][currentCol].getName().equalsIgnoreCase("batt")){
						batts++;
					}
	...
	// While I talked about not including a death mechanic
	// I decided to include on, it has no long term punishment though 
	private void die() {
		currentX=ROBOT_START_X;
		currentY=GameFrame.HEIGHT-PlayPanel.TERRAIN_HEIGHT-ROBOT_HEIGHT;
		currentCol=currentX/Tile.TILE_SIZE;
		currentRow=currentY/Tile.TILE_SIZE;
		boundingBox=new Rectangle(ROBOT_START_X+DISPLACEMENT,currentY,ROBOT_WIDTH,ROBOT_HEIGHT);
		last_direction=KeyEvent.VK_RIGHT;
	...
	//sets the current frame when the robot is moving - we have a total of 5 frames for 
	//each run direction. The variable moveCounter is incremented each time the gameManager
	//calls the move function on the Robot. So according to moveCounter we can choose the current
	...
	//if the character is not already jumping (boolean jumping=true)
	public void jump() {
	...
	//initially the protag is not falling
	...
	public static final int ROBOT_START_X=64;
	
	//height of the main character (used to set the boundingBox)
	private final int ROBOT_HEIGHT=64;
	
	//width of the main character (used to set the boundingBox)
	private final int ROBOT_WIDTH=32;
	
	//current position of the character along the x-axis 
	//initially the character is placed at ROBOT_START_X
	private int currentX=ROBOT_START_X;
	
	//current position of the character along the y-axis 
	//initially the character is placed at ROBOT_START_X
	private int currentY=GameFrame.HEIGHT-PlayPanel.TERRAIN_HEIGHT-ROBOT_HEIGHT;
	...
	//batteries owned by the character. The robot starts with no batteries
	private int batts=0;
}
